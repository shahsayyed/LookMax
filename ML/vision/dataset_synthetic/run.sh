...
case "$1" in
    ...
    benchmark)
        echo "==> Running hardware mode benchmark (comparing sequential vs batched vs concurrent)..."
        python3 "$SCRIPT_DIR/full_run.py" --compare-modes --data-dir "$DATA_DIR"
        ;;
    ...
    start)
        WORKERS="${2:-2}"
        echo "==> Launching full $TOTAL_TARGET image generation with $WORKERS worker(s)..."
        mkdir -p "$IMAGES_DIR"
        ...
        if command -v tmux >/dev/null 2>&1; then
            ...
            tmux new-session -d -s "$TMUX_SESSION" \
                "export HF_HOME='$HF_HOME'; export HF_HUB_ENABLE_HF_TRANSFER=1; export LOOKMAX_DATA_DIR='$DATA_DIR'; cd '$SCRIPT_DIR'; python3 full_run.py --num-workers $WORKERS --data-dir '$DATA_DIR'; echo 'Generation finished. Press Enter to close.'; read"
            ...
            nohup python3 "$SCRIPT_DIR/full_run.py" --num-workers "$WORKERS" --data-dir "$DATA_DIR" > "$OUTPUT_DIR/generation.log" 2>&1 &
    ...
    status)
        ...
        echo " Completed   : $DONE_COUNT / $TOTAL_TARGET images ($PERCENT%)"
    ...
    *)
        ...
        echo "  benchmark          Benchmark sequential vs batched vs concurrent workers"
        echo "  variation [N]      Run 64-image diversity check (default: 2 workers)"
        echo "  start [N]          Start full 28,000 generation with N workers in tmux (default: 2)"
